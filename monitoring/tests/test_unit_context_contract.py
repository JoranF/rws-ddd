...
def test_monitoring_context_has_required_layer_scaffold() -> None:
...
def test_monitoring_context_documents_core_domain_language() -> None:
    for term in (
        "Sensor",
        "SensorData",
        "MonitoringSessie",
...
def test_monitoring_context_boundary_is_explicit() -> None:
    assert "Bezit:" in GUIDANCE
    assert "Bezit NIET:" in GUIDANCE
    assert "Verwijs naar het kunstwerk via `kunstwerkId`" in GUIDANCE
