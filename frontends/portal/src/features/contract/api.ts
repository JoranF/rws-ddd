...
// proxypad /contract. Types spiegelen de weergave-DTO's van de service.
...
export const contractApi = {
  ...
  aanbesteding: (id: string) => api.get<AanbestedingWeergave>(`/svc/contract/api/aanbestedingen/${id}`),
  ...
  dienInschrijvingIn: (id: string, body: { aannemer: string; prijs: number; kwaliteitsscore: number }) =>
    api.post<{ status: string }>(`/svc/contract/api/aanbestedingen/${id}/inschrijvingen`, body),
  gun: (id: string, body: { looptijdStart: string; looptijdEind: string }) =>
    api.post<{ contractId: string }>(`/svc/contract/api/aanbestedingen/${id}/gunning`, body),
  contracten: (kunstwerkId?: string) =>
    api.get<ContractWeergave[]>(`/svc/contract/api/contracten${kunstwerkId ? `?kunstwerkId=${kunstwerkId}` : ''}`),
  contract: (id: string) => api.get<ContractWeergave>(`/svc/contract/api/contracten/${id}`),
  dienPrestatieverklaringIn: (id: string, body: { periodeStart: string; periodeEind: string; bedrag: number }) =>
    api.post<{ status: string }>(`/svc/contract/api/contracten/${id}/prestatieverklaringen`, body),
  rondAf: (id: string, body: { datum: string }) =>
    api.post<{ status: string }>(`/svc/contract/api/contracten/${id}/afronding`, body),
};
